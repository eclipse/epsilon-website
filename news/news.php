...
include_once(Epsilon::getIncludeLocation()."/dom4php/XmlParser.php");
...
function r2h($title, $feed, $posts, $heading=3, $limit = 5, $chopAt = 400) {
	...
	$html = '<h'.$heading.'>';
	$html .= "<a href=\"$feed\" class=\"pull-right\"><img src=\"/img/rss.gif\" alt=\"More...\" /></a>";
	...
	foreach ($channel->selectElements(array(),"item") as $item) {
		...
		foreach ($posts as $post) {
			if (($post == "" || substr_count($title,$post) > 0) && $i < $limit) {
				...
				$html .= '   <span style="color:#939393; font-size:smaller;"><i>('.$item->getOneChild("pubDate")->childNodes[0]->data.')</i></span>';
				//$html .= '   <span style="color:#939393; font-size:smaller;"><i>('.substr($item->getOneChild("pubDate")->childNodes[0]->data, 0, 16).')</i></span>';
...
function rdf2h($title, $feed, $posts, $heading=3, $limit = 5, $chopAt = 400) {
	...
	$channel = $document->documentElement;
	$html = '<h'.$heading.'>';
	$html .= "<a href=\"$feed\" style=\"float:right\"><img src=\"/img/rss.gif\" alt=\"More...\" /></a>";
